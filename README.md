...
- **노트패드 방식** — 여러 줄을 적으면 각 줄 끝에 결과가 바로 표시됩니다.
...
- **자동완성** — 변수(한글 포함)·단위·통화·함수·키워드를 입력 중 제안합니다.
- **단위 변환** — `3 km to m`, `1 hour in minutes` 등.
- **통화 변환** — `10 USD to KRW`, `2 EUR + 3 EUR`. 시작 시 실시간 환율을 불러오며, 실패하면 고정값으로 폴백합니다(아래 참고).
- **백분율** — `20% of 50`, `100 + 10%` 같은 자연어 표현.
- **합계** — 숫자 결과들의 합을 헤더에 실시간 표시(단위 값은 제외).
- **클릭 복사** — 줄 끝 결과를 클릭하면 클립보드로 복사됩니다.
...
- [CodeMirror 6](https://codemirror.net/) — 에디터(문법 하이라이팅·자동완성·인라인 결과)

## 참고

- **환율**은 앱 시작 시 [open.er-api.com](https://www.exchangerate-api.com/docs/free)에서
  실시간으로 불러와 브라우저에 캐시합니다. 네트워크 실패 시
  `src/helpers/currency.ts`의 고정 폴백값으로 동작하므로 오프라인에서도 계산됩니다.
